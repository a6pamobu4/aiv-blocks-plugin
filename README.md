...
All custom styles are authored in SCSS. Do not edit generated CSS files manually.

For Gutenberg blocks, keep styles inside each block folder:

-   `style.scss` for frontend and shared editor styles.
-   `editor.scss` for editor-only styles.

The build process compiles block SCSS into `build/blocks/{block-name}/`:
...
Build production CSS and JavaScript before committing or packaging:

```bash
npm run build
```

Compiled CSS is generated by `@wordpress/scripts` from SCSS. This plugin does not enqueue SCSS files and does not use Bootstrap, Tailwind, CSS frameworks, external fonts, or external icon libraries.

For theme projects, keep the WordPress-required `style.css` file intact when it is needed for theme metadata. Author theme styles in `assets/scss/`, compile them to `assets/css/`, and enqueue only the compiled CSS from PHP.
